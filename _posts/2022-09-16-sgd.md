...
Let's get back to the original [**SGD**](#SGD), we can consider a **sum of squares** term $$SS(\nabla)$$ for the **gradient** $$-\nabla_\theta \mathcal{L}(\theta_t)$$,

$$SS(\nabla) = \sum_{T=0}^t \left( \nabla_\theta \mathcal{L}(\theta_T) \right)^2$$

where we basically add the square of **gradients** $$\nabla^2$$ from time $$T=0$$ to $$T=t$$.

The algorithm becomes, 

$$\theta_{t+1} = \theta_t - \frac{\alpha \nabla_\theta \mathcal{L}(\theta_t)}{\sqrt{SS(\nabla)} + 0.1^7}$$
